...
export default function LoginScreen({ navigation }) {
  ...
  const dynamicStyles = useMemo(() => ({
    ...
    emptyTitle:{ ...typography.title2,   color: colors.text, marginTop: spacing.md },
    emptyText: { ...typography.body,     color: colors.textMuted, textAlign: 'center', marginTop: spacing.sm, paddingHorizontal: spacing.lg },
  }), [colors]);
      ...
      <View style={styles.body}>
        {users.length === 0 ? (
          <View style={styles.emptyWrap}>
            ...
            <Text style={dynamicStyles.emptyTitle}>Welcome!</Text>
            <Text style={dynamicStyles.emptyText}>No accounts yet. Create one to start tracking expenses.</Text>
          </View>
        ) : (
          <>
            <Text style={dynamicStyles.sectionLabel}>Choose your account</Text>
            <FlatList
              data={users}
              keyExtractor={(u) => u.id}
              ...
              renderItem={({ item }) => (
                <Pressable
                  ...
                  style={({ pressed }) => [dynamicStyles.userRow, pressed && { opacity: 0.7 }]}
                >
...
const styles = StyleSheet.create({
  ...
  heroTitle:    { ...typography.largeTitle, color: '#FFFFFF' },
  heroSubtitle: { ...typography.body, color: 'rgba(255,255,255,0.85)', marginTop: spacing.xs },
  ...
  avatarText: { ...typography.title3, color: '#FFFFFF' },
