...
import { spacing, typography, CATEGORY_META, shadows } from '../theme';
...
export default function DashboardScreen({ navigation }) {
  ...
  function handleEdit(item) { navigation.navigate('Add', { expense: item }); }
  ...
  const topCategories = Object.entries(summary.byCategory || {}).filter(([, v]) => v.debit > 0).sort((a, b) => b[1].debit - a[1].debit).slice(0, 5);
  const netPositive = (summary.net || 0) >= 0;
  const recentFive = [...expenses].reverse().slice(0, 5);
  const categoryItems = activeCategory ? expenses.filter(e => e.category === activeCategory) : [];
    ...
    <>
      ...
      >
        ...
        <View style={styles.row}>
          ...
          <View style={{ width: spacing.md }} />
          <StatCard label="Top category" value={topCategories[0]?.[0] || '—'} icon="trophy-outline" />
        </View>

        <RecentTransactions items={recentFive} deletingId={deletingId} onEdit={handleEdit} onDelete={handleDelete} styles={styles} colors={colors} />
        <TopSpending top={summary.topSpending} styles={styles} />
        <CategoryBreakdown byCategory={summary.byCategory} onSelect={(cat) => setActiveCategory(cat)} styles={styles} colors={colors} />
...
function RecentTransactions({ items, deletingId, onEdit, onDelete, styles, colors }) {
  return (
    <>
      <Text style={styles.section}>Recent transactions</Text>
      {items.length === 0 ? (
        <Card><Text style={styles.empty}>No transactions this month yet.</Text></Card>
      ) : (
        <Card padded={false}>
          {items.map((item, i) => {
              ...
              <View key={item.id}>
                ...
                <View style={styles.recentRow}>
                  ...
                  <View style={{ flex: 1, minWidth: 0 }}>
                    <Text style={styles.recentTitle} numberOfLines={1}>{item.details || '—'}</Text>
                    <Text style={styles.recentMeta}>{item.date} · {item.category}</Text>
                  </View>
                  <Text style={[styles.recentAmount, { color: isCredit ? colors.success : colors.danger }]}>
                    {isCredit ? '+' : '-'}{inr(item.amount)}
                  </Text>
                  <TouchableOpacity onPress={() => onEdit(item)} style={styles.actionBtn} hitSlop={8} disabled={isDeleting}>
                    <Ionicons name="pencil-outline" size={16} color={colors.primary} />
                  </TouchableOpacity>
                  <TouchableOpacity onPress={() => onDelete(item)} style={[styles.actionBtn, { marginLeft: 2 }]} hitSlop={8} disabled={isDeleting}>
                    {isDeleting
    ...
    <Modal visible={visible} transparent animationType="none" statusBarTranslucent onRequestClose={onClose}>
      ...
      <Animated.View style={[overlayStyles.sheet, { transform: [{ translateY: slideAnim }], paddingBottom: insets.bottom + spacing.lg }]}>
        ...
        <View style={overlayStyles.summaryRow}>
          <SummaryChip label="Credit" value={inr(credit)} color={colors.success} overlayStyles={overlayStyles} colors={colors} />
          <SummaryChip label="Debit"  value={inr(debit)}  color={colors.danger}  overlayStyles={overlayStyles} colors={colors} />
          <SummaryChip label="Net" value={inr(credit - debit)} color={credit - debit >= 0 ? colors.success : colors.danger} overlayStyles={overlayStyles} colors={colors} />
        </View>
...
function SummaryChip({ label, value, color, overlayStyles, colors }) {
  return (
...
function makeStyles(colors) {
  return StyleSheet.create({
    ...
    recentRow: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: spacing.lg, paddingVertical: spacing.md, gap: spacing.sm },
    recentIcon: { width: 36, height: 36, borderRadius: 12, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
    ...
    actionBtn: { width: 30, height: 30, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
    ...
    rowSep:  { height: StyleSheet.hairlineWidth, backgroundColor: colors.separator, marginLeft: spacing.lg + 36 + spacing.sm },
